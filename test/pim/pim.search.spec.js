...
describe("PIM - employee search",function(){
  ...
  it("should display employee list",async function(){
    expect((await driver.findElements(By.css(".oxd-table-body .oxd-table-row"))).length).to.be.greaterThan(0);
  });

  it("should display employee search filters",async function(){
    expect((await driver.findElements(By.css("input"))).length).to.be.greaterThan(0);
  });
